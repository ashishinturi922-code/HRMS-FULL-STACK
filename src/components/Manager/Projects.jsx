...
const ManagerProjects = () => {
  ...
  // ✅ FIXED - now reads the correct variable used everywhere else
  ...
  const handleOpenEmployeeModal = (projectId) => {
    ...
    // ✅ FIX: employeeIds now stores employee_id strings (e.g. "ACS1001")
  ...
  const handleEmployeeToggle = (employeeCode) => {
    setSelectedEmployees((prev) => {
      ...
      const updated = current.includes(employeeCode)
        ? current.filter((id) => id !== employeeCode)
  ...
  const handleSelectAll = () => {
    ...
    setSelectedEmployees((prev) => ({
      ...prev,
      [currentProjectId]: allEmployeeCodes,
    }));
  };
  ...
  const handleAssignEmployees = async () => {
    ...
    try {
      setLoading(true);
      console.log("Assigning employees (employee_id codes):", selectedIds);
  ...
  const handleRemoveEmployee = async (projectId, employeeCode) => {
    ...
    try {
      setLoading(true);
      const project = projects.find((p) => p.id === projectId);
      const currentIds = (project?.employeeIds || []).filter(
        (id) => String(id) !== String(employeeCode)
      );
            ...
            <tbody>
              {filteredProjects.length > 0 ? (
                filteredProjects.map((p, index) => (
                  <tr
                    key={p.id}
                    className={p.status === "Completed" ? "completed-row" : ""}
                  >
                    <td>{index + 1}</td>
                ...
                filteredProjects.map((p, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        <span
                          className={`status ${(p.status || "ongoing")
                            .toLowerCase()
                            .replace(/\s+/g, "-")}`}
                        >
                          {p.status || "Ongoing"}
          ...
          <div className="modal-content">
            ...
            <div className="modal-search">
              ...
              <div className="modal-search-info">
                Selected: <strong>{currentSelection.length}</strong> /{" "}
                {employees.length}
              </div>
            </div>
                    ...
                    >
                      <label className="employee-checkbox">
                        ...
                        <span className="employee-info">
                          ...
                          <br />
                          <small>
                            {emp.employee_id || emp.id} • {emp.role}
                          </small>
                        </span>
                      </label>
                    </div>
            ...
            <div className="modal-buttons-bottom">
              ...
              >
                {loading
                  ? "⏳ Assigning..."
                  : `✓ Assign (${currentSelection.length})`}
              </button>
            </div>
          </div>
        </div>
