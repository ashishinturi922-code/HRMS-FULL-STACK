...
app.get('/api/health', async (req, res) => {
    try {
        ...
        res.json({ status: 'Server is running ✅', database: 'Connected ✅', timestamp: new Date().toISOString(), port: 5000 });
    } catch (err) {
        res.status(500).json({ status: 'Server is running ✅', database: `❌ DB Error: ${err.message}`, timestamp: new Date().toISOString() });
    }
});
...
app.get('/api/fix-password/:username/:pass', async (req, res) => {
    try {
        ...
        const [result] = await db.execute('UPDATE users SET password = ? WHERE username = ?', [hashed, req.params.username]);
        if (result.affectedRows === 0) return res.status(404).send(`❌ User '${req.params.username}' not found`);
...
app.get('/api/profile/:id',               (req, res) => AdminController.getProfile(req, res));
app.put('/api/profile/:id',               (req, res) => AdminController.updateProfile(req, res));
app.post('/api/profile/upload-photo/:id', upload.single('photo'),    (req, res) => AdminController.uploadPhoto(req, res));
app.post('/api/profile/upload-doc/:id',   upload.single('document'), (req, res) => AdminController.uploadDoc(req, res));
app.post('/api/profile/update-password',  (req, res) => AdminController.updatePassword(req, res));
...
app.put('/api/manager/update-profile',                                  (req, res) => ManagerController.updateProfile(req, res));
app.post('/api/manager/update-password',                                (req, res) => ManagerController.updatePassword(req, res));
app.post('/api/manager/upload-photo/:id',   upload.single('photo'),     (req, res) => ManagerController.uploadPhoto(req, res));
app.post('/api/manager/upload-doc',         upload.single('document'),  (req, res) => ManagerController.uploadDoc(req, res));
...
app.post('/api/teamleader/change-password',                          (req, res) => TeamLeaderController.updatePassword(req, res));
app.post('/api/teamleader/upload-photo/:id', upload.single('photo'), (req, res) => TeamLeaderController.uploadPhoto(req, res));
...
app.get('/api/employee/resignation/:empId',                          (req, res) => EmployeeController.getMyResignation(req, res));
app.post('/api/employee/resignation',                                (req, res) => EmployeeController.submitResignation(req, res));
app.get('/api/employee/offers/:empId',                               (req, res) => EmployeeController.getMyOffers(req, res));

// =================================================================
//  WFH — WORK FROM HOME ROUTES
//  URLs match the frontend fetch calls exactly
// =================================================================

// ── EMPLOYEE WFH ─────────────────────────────────────────────────
// EmployeeApplyLeave.jsx → fetchWFH()   GET  /api/employee/wfh/:empId
app.get('/api/employee/wfh/:empId',                                  (req, res) => WFHController.getMyWFH(req, res));
// EmployeeApplyLeave.jsx → handleSubmit POST /api/employee/apply-wfh
app.post('/api/employee/apply-wfh',                                  (req, res) => WFHController.applyWFH(req, res));
// EmployeeApplyLeave.jsx → handleDelete DELETE /api/employee/delete-wfh/:id
app.delete('/api/employee/delete-wfh/:wfhId',                        (req, res) => WFHController.deleteWFH(req, res));

// ── TEAM LEADER — own WFH ─────────────────────────────────────────
// TeamLeaderApplyLeave.jsx → fetchWFH()    GET  /api/tl-leave/my-wfh/:tlId
app.get('/api/tl-leave/my-wfh/:tlId',                                (req, res) => WFHController.tlGetMyWFH(req, res));
// TeamLeaderApplyLeave.jsx → handleSubmit  POST /api/tl-leave/apply-wfh
app.post('/api/tl-leave/apply-wfh',                                  (req, res) => WFHController.tlApplyWFH(req, res));
// TeamLeaderApplyLeave.jsx → handleDelete  DELETE /api/tl-leave/delete-wfh/:id
app.delete('/api/tl-leave/delete-wfh/:wfhId',                        (req, res) => WFHController.tlDeleteWFH(req, res));

// ── TEAM LEADER — approves employee WFH ──────────────────────────
// TL views pending employee WFH requests
app.get('/api/teamleader/wfh/pending/:tlId',                         (req, res) => WFHController.tlGetPendingEmployeeWFH(req, res));
// TL approves / denies  → status: 'TL Approved' | 'Denied'
app.put('/api/teamleader/wfh/approve/:wfhId',                        (req, res) => WFHController.tlApproveEmployeeWFH(req, res));

// ── MANAGER — own WFH ─────────────────────────────────────────────
// ApplyLeave.jsx (Manager) → fetchWFH()    GET  /api/manager/my-wfh/:managerId
app.get('/api/manager/my-wfh/:managerId',                            (req, res) => WFHController.managerGetMyWFH(req, res));
// ApplyLeave.jsx (Manager) → handleSubmit  POST /api/manager/apply-wfh
app.post('/api/manager/apply-wfh',                                   (req, res) => WFHController.managerApplyWFH(req, res));
// ApplyLeave.jsx (Manager) → handleDelete  DELETE /api/manager/delete-wfh/:id
app.delete('/api/manager/delete-wfh/:wfhId',                         (req, res) => WFHController.managerDeleteWFH(req, res));

// ── MANAGER — approves TL WFH ─────────────────────────────────────
// Manager sees TL WFH queue
app.get('/api/tl-wfh/pending/:managerId',                            (req, res) => WFHController.tlGetPendingForManager(req, res));
// Manager approves / denies TL WFH  → status: 'Approved' | 'Denied'
app.put('/api/tl-wfh/approve/:wfhId',                                (req, res) => WFHController.tlApproveWFH(req, res));

// ── MANAGER — final-approves employee WFH ─────────────────────────
// Manager sees employee WFH requests already TL-approved
app.get('/api/manager/wfh/pending-employees/:managerId',             (req, res) => WFHController.managerGetPendingEmployeeWFH(req, res));
// Manager final-approves / denies  → status: 'Approved' | 'Denied'
app.put('/api/manager/wfh/approve/:wfhId',                           (req, res) => WFHController.managerApproveEmployeeWFH(req, res));

// ── ADMIN — approves MANAGER WFH ──────────────────────────────────
// Admin sees pending manager WFH requests
app.get('/api/admin/wfh/pending-managers',                           (req, res) => WFHController.adminGetPendingManagerWFH(req, res));
// Admin approves / denies Manager WFH → status: 'Approved' | 'Denied'
app.put('/api/admin/wfh/approve/:wfhId',                             (req, res) => WFHController.adminApproveManagerWFH(req, res));
